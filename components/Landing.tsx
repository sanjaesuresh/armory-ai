...
// ── Role colour identity ──────────────────────────────────────────────────────
// Maps each role id to a pastel tint bg + its saturated accent companion.
// All bg/ink combos: --ink (#272319) on each tint ≥ 11:1 — WCAG AAA.
// All bg/accent combos: per-accent on its tint ≥ 4.5:1 — WCAG AA (see globals.css).
const ROLE_COLORS: Record<string, { bg: string; accent: string }> = {
  'marketing-manager':    { bg: 'var(--butter)', accent: 'var(--accent-butter)' },
  'small-business-owner': { bg: 'var(--peach)',  accent: 'var(--accent-peach)'  },
  'customer-support':     { bg: 'var(--sky)',    accent: 'var(--accent-sky)'    },
  'recruiter':            { bg: 'var(--lilac)',  accent: 'var(--accent-lilac)'  },
  'sales-rep':            { bg: 'var(--mint)',   accent: 'var(--accent-mint)'   },
  'operations':           { bg: 'var(--sage)',   accent: 'var(--accent-sage)'   },
  'founder-generalist':   { bg: 'var(--blush)',  accent: 'var(--accent-blush)'  },
};

// ── Featured setups (server-side, graceful fallback) ─────────────────────────
// Rendered as a separate async sub-component so Landing itself stays sync
// (safe for page.tsx which wraps it with a plain JSX return).
async function FeaturedSection() {
  let featured: Setup[] = [];
  try {
    const repo = createCatalogRepository();
    const all = await repo.getSetups();
    featured = all.filter((s) => s.featured !== null).slice(0, 3);
  } catch {
    // DB unavailable (e.g. env vars not set) — section is silently omitted.
  }

  if (featured.length === 0) return null;

  return (
    <section className="section zone-tint tint-lilac">
      <div className="wrap">
        <SectionHead
          eyebrow="Featured setups"
          title="Popular with professionals like you"
          action={
            <Link className="btn btn-ghost btn-sm" href="/professionals">
              See all setups
            </Link>
          }
        />
...
export default function Landing() {
  return (
    <main>
      ...
      {/* ── Hero — warm butter zone (tinted focal anchor) ──────────────── */}
      <section className="hero zone-tint tint-butter">
        <div className="wrap hero-grid">
          ...
          {/* Right illustration — raised focal panel (shadow-3 + ambient lift) */}
          <div className="hero-art">
            <div
              style={{
                background: 'var(--surface-raised)',
                borderRadius: 'var(--r-lg)',
                boxShadow: 'var(--shadow-3), var(--shadow-ambient)',
                padding: '8px',
              }}
            >
              <svg
      ...
      {/* ── Role section — white canvas, colourful accented cards ─────── */}
            ...
            {ROLES.map((role) => {
              ...
              return (
                <Link
                  ...
                  style={{
                    background: colors?.bg ?? 'var(--paper)',
                    '--cat-accent': colors?.accent ?? 'transparent',
                  } as React.CSSProperties}
                >
                  {/* White icon badge pops off any tint */}
                  <span
                  ...
                  {/* ink-soft instead of muted: --muted fails WCAG AA on pastel tints */}
                  <span style={{ color: 'var(--ink-soft)' }}>{role.description}</span>
                </Link>
              );
            })}
            <Link
      ...
      {/* ── How it works — sky tint zone ──────────────────────────────── */}
      <section className="section zone-tint tint-sky" id="how">
        <div className="wrap">
      ...
      {/* ── What you actually get — white canvas ──────────────────────── */}
...
Tone of voice: Conversational`}</pre>
      ...
      {/* ── Featured setups — lilac tint (async, silently omitted if DB unavailable) */}
      <FeaturedSection />

      {/* ── Export, don't host — mint tint close ─────────────────────── */}
      <section className="section zone-tint tint-mint">
